...
public class IndexEntry implements Comparable<IndexEntry> {
     ...
     * two modes is that when the strings are equal comparing them in case-insensitive
     * manner in {@link #WORD_MATCH} mode then the further comparison
    ...
    private int compareToPrefix(String prefix) {
        ...
        if (lemma.length() > prefix.length()) {
            truncatedWord = lemma.substring(0, prefix.length());
        } else {
            truncatedWord = lemma;
        }
